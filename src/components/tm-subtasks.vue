    ...
    <div class="subtasks-title">
      ...
      >
        ...
        <div
            ...
            @click="editingSubtask=true;indexDel=index;deleteOrNoDelete=false;creatingSubtask=false;"
        >{{subtask}}
        </div>
        <div v-if="editingSubtask&&index===indexDel" style="display: flex">
        ...
        <svg @click="deleteOrNoDelete=true;indexDel=index" v-else style="cursor: pointer" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default {
  ...
  methods: {
    createSubtask(bool) {
      if (bool) {
        const newSubtask = {
          index: this.whereSubTasksShow ? this.index : this.TASKS.length - 5 + this.index,
    ...
    deleteSubtask(bool,index) {
      if(bool) {
        const deleteSubtask = {
          indexTask: this.whereSubTasksShow ? this.index : this.TASKS.length - 5 + this.index,
          indexSubtask: index
        }
        this.$store.dispatch('deleteSubtask', deleteSubtask);
      }
      this.deleteOrNoDelete=false;
    }
  },
  computed: {
    ...mapGetters([
