<template>
  <div class="tm-subtasks">
    <div
      ...
      <svg v-if="subtask.status" width="16" height="16" viewBox="0 0 12 12" fill="none"
      ...
      <svg v-if="!subtask.status" width="16" height="16" viewBox="0 0 16 16" fill="none"
           xmlns="http://www.w3.org/2000/svg">
        <path
      ...
      <div class="select-status-for-task" v-show="openThisTableStatuses===indexSubTask">
        <div
            style="color: #576F9D"
            :style="{background: !subtask.status ? '#E5EBF3': ''}"
            class="container-for-statuses"
            @click="selectThisStatus(false)"
        >Нужно сделать
          <svg v-show="!subtask.status" width="10" height="10" viewBox="0 0 10 10" fill="none"
               xmlns="http://www.w3.org/2000/svg">
        ...
        <div
            ...
            :style="{background: subtask.status ? '#DFFCDB': ''}"
